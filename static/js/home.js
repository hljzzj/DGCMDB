...
var MAC = {
    ...
    'Login': {
        ...
        'Info': function() {
            $.ajax({
                type: 'get',
                url: SitePath + 'index.php?m=user-ajaxinfo',
                dataType: 'json',
                timeout: 5000,
                error: function() {
                    alert('登录失败');
        ...
        'Out': function($id) {
            $.ajax({
                type: 'get',
                url: SitePath + 'index.php?m=user-logout',
                timeout: 5000,
                success: function($r) {
        ...
        'In': function() {
            ...
            $.ajax({
                type: 'post',
                url: SitePath + 'index.php?m=user-check',
                data: 'u_name=' + $("#username").val() + '&u_password=' + $("#userpass").val(),
                timeout: 5000,
                error: function() {
                    alert('登录失败');
                },
                    ...
                    parse: function(obj) {
                        if (obj.status) {
                            ...
                            for (var i = 0; i < obj.data.length; i++) {
                                parsed[i] = {
                                    data: obj.data[i],
                                    value: obj.data[i].d_name,
                                    result: obj.data[i].d_name
                                };
                            }
                            return parsed;
                        } else {
    ...
    'Score': {
        ...
        'Show': function($f, $tab, $id) {
            ...
            if ($f == 1) {
                ...
                for (var i = 1; i <= 10; i++) {
                    str += '<li><a value="' + i + '" class="star_' + i + '">' + i + '</a></li>';
                }
                str += '</ul>';
            ...
            $.ajax({
                type: 'get',
                url: SitePath + this.ajaxurl + '&tab=' + $tab + '&id=' + $id,
                timeout: 5000,
                error: function() {
                    alert('评分加载失败');
                    $(".star").html('评分加载失败');
    ...
    'Hits': function(tab, id) {
        $.get(SitePath + "inc/ajax.php?ac=hits&tab=" + tab + "&id=" + id,
        function(r) {
            $('#hits').html(r);
        });
    },
    ...
    'UserFav': function(id) {
        $.get(SitePath + "inc/ajax.php?ac=userfav&id=" + id + "&rnd=" + Math.random(),
        function(r) {
            if (r == "ok") {
                alert("会员收藏成功");
            } else if (r == "login") {
